"""
download_puts_expanded.py
─────────────────────────────────────────────────────────────────────────────
Downloads put option history for the 25 Tier 1 + Tier 2 expansion tickers
identified by analyze_score_coverage.py.

Identical logic to download_calls_expanded.py except SIDE='put' and the
strike target is 8% BELOW the underlying (not above).

Usage on server:
  cd /root/trading-bot
  nohup python3 -u download_puts_expanded.py > /tmp/puts_expanded.log 2>&1 &
"""
...
TARGET_DTE     = 35     # puts use 35 DTE (live bot's CSP target)
OTM_PCT        = 0.08   # 8% OTM for put strike (strike BELOW underlying)
RATE_LIMIT_RPS = 5
SLEEP_BETWEEN  = 1.0 / RATE_LIMIT_RPS
SIDE           = 'put'
YEARS          = 5

# Tier 1 (10) + Tier 2 (15) = 25 tickers
UNIVERSE = [
    # Tier 1
...
def init_db(db_path):
    ...
    ''')
    conn.execute('''
        CREATE TABLE IF NOT EXISTS fetch_errors_puts_expanded (
...
def already_fetched(conn, symbol, trade_date):
    cur = conn.execute(
        "SELECT 1 FROM options_history WHERE symbol=? AND trade_date=? AND side='put' LIMIT 1",
        (symbol, trade_date))
    if cur.fetchone():
        return True
    cur = conn.execute(
        'SELECT 1 FROM fetch_errors_puts_expanded WHERE symbol=? AND trade_date=? LIMIT 1',
        (symbol, trade_date))
...
def save_error(conn, symbol, trade_date, error):
    conn.execute('''
        INSERT OR REPLACE INTO fetch_errors_puts_expanded
...
def find_target_expiration(expirations, trade_date_str, target_dte=35):
...
def download():
    ...
    print(f"[*] EXPANDED PUT Downloader")
    print(f"   Tickers: {len(UNIVERSE)} | Days: {len(trade_days)} | Total: {total:,}")
    print(f"   Side:    {SIDE} ({OTM_PCT*100:.0f}% OTM BELOW underlying, ~{TARGET_DTE} DTE)")
    print(f"   DB:      {DB_PATH}")
    print()
    ...
    for ticker_idx, ticker in enumerate(UNIVERSE, 1):
        ...
        for trade_date in trade_days:
            ...
            try:
                ...
                # PUT: target strike is BELOW underlying (-8%)
    ...
    print(f"\n{'='*60}")
    print(f"[+] Expanded put download complete")
